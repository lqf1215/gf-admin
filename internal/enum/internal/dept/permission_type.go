...
var (
	...
	PermissionType    = func(modules co_interface.IModules) *permissionType[co_interface.IModules] {
		result := permissionTypeMap.GetOrSet(modules.GetConfig().KeyIndex, &permissionType[co_interface.IModules]{
			...
			ViewDetail: permission.NewInIdentifier("ViewDetail", "详情", "查看部门详情"),
			List:       permission.NewInIdentifier("List", "列表", "查看部门列表"),
			Create:     permission.NewInIdentifier("Create", "新增", "新增部门信息"),
			Update:     permission.NewInIdentifier("Update", "更新", "更新部门信息"),
			Delete:     permission.NewInIdentifier("Delete", "删除", "删除部门信息"),
			SetState:   permission.NewInIdentifier("SetState", "设置状态", "设置部门状态"),
			ListTree:   permission.NewInIdentifier("ListTree", "树形列表", "查看部门树形列表"),
		})
